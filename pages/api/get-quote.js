	...
	if (req.method === "POST") {
		try {
			...
			if (!Number(id)) {
				return res
					.status(401)
					.json({ status: false, message: "Select a product" });
			}
			...
			if (validator.isEmpty(fullname)) {
				return res
					.status(401)
					.json({ status: false, message: "Please, enter your name" });
			} else if (!validator.isAlpha(fullname, "en-US", { ignore: " " })) {
				return res
					.status(401)
					.json({ status: false, message: "Invalid name character" });
			}

			if (!validator.isEmail(email)) {
				return res
					.status(401)
					.json({ status: false, message: "Invalid E-mail address" });
			}

			if (
				!validator.isEmpty(message) &&
				!validator.isAlphanumeric(message, "en-US", { ignore: " " })
			) {
				return res
					.status(401)
					.json({ status: false, message: "Invalid characters in message" });
			}
