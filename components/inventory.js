...
function Inventory() {
	...
	const [displayedInventory, setDisplayedInventory] = useState([...machines.slice(0, 3)])

	useEffect(() => {
		if (filter.length > 0) {
			const filteredInventory = machines.filter((machine) => machine.type === filter)
		...
		<section className='inventory'>
			<div className='content-wrapper'>
				<div className='inventory__header'>
					...
					<p>
						Our machines are available in different sizes and shapes. When you need power and durability, our equipment is
						your best choice.
					</p>
				</div>
				...
				<AnimatePresence>
					{displayedInventory.map((inventory) => (
						<motion.li
							...
							<div className='listing-label'>{inventory.specification.Condition}</div>
							<div className='inventory__list__item--content'>
								...
								<div className='d-flex'>
									...
									<div>
										<button
											onClick={() => showModalWithProduct(inventory.id, inventory.type, inventory.title)}
								...
								<ul className='inventory__meta'>
									...
									<li className='inventory__meta__item' title='Undercarriage'>
									...
									<li className='inventory__meta__item' title='manufactured date'>
										<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width={24} height={24}>
											<path fill='none' d='M0 0h24v24H0z' />
