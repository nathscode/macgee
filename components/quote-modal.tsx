...
import { QuoteModalContext } from "./layout/provider";
...
function QuoteModal() {
	...
	const { data: docData, isLoading: isLoadingDoc, error } = useFetchDocuments();

	useEffect(() => {
		if (!isLoadingDoc && docData) {
			// console.log(docData);
			setMachines(docData);
			setTypes([...new Set(docData.map((machine) => machine.category))]);
		}
	}, [docData, isLoadingDoc, error]);

	// const types = [...new Set(machines.map((machine) => machine.type))];
	useEffect(() => {
		if (data.type.trim() !== "") {
			const newItems = machines.filter((i) => i.category === data.type);
			setItems(newItems);
		}
	}, [data.type]);
